...
namespace gBanker.Web.Controllers
{
    public class SpecialLoanCollectionController : BaseController
    {
        ...
        public JsonResult GetOldPurpose(int officeId, int centerId, long MemId, int ProdId)
        {
            var param = new { OfficeID = officeId, CenterID = centerId, MemberID = MemId, ProductID = ProdId };
            var getPurpose = ultimateReportService.GetDataWithParameter(param, "GetOldPurpose");
            var oldPurpose = getPurpose.Tables[0].AsEnumerable().Select(p => new SpecialLoanCollectionViewModel
            {
                PurposeID = p.Field<short>("PurposeID"),
                PurposeName = p.Field<string>("PurposeName"),
                LoanSummaryID = p.Field<long>("LoanSummaryID")
            });
            return Json(oldPurpose, JsonRequestBehavior.AllowGet);
        }
        public ActionResult GetPurposeList(short productID, string purposeCode)
        {
            var mainProductCode = "";
            var mainProduct = productService.GetMany(p => p.ProductID == productID).FirstOrDefault();
            if (mainProduct != null)
                mainProductCode = mainProduct.MainProductCode;
            var param = new { PurposeCode = purposeCode, MainProductCode = mainProductCode };
            var dataList = ultimateReportService.GetDataWithParameter(param, "Get_AutoCompletePurposeCode");
            var datasetToDatatable = dataList.Tables[0];
            var viewPurposeList = datasetToDatatable.AsEnumerable().Select(p => new LoanApprovalViewModel
            {
                PurposeID = Convert.ToInt16(p.Field<int>("PurposeID")),
                PurposeName = p.Field<string>("PurposeName")
            }).ToList();
            return Json(viewPurposeList, JsonRequestBehavior.AllowGet);
        }
        public JsonResult UpdatePurpose(long loanSummaryID, short purposeId)
        {
            int result = 0;
            var message = "";
            var param = new { LoanSummaryID = loanSummaryID, PurposeID = purposeId };
            ultimateReportService.GetDataWithParameterCollection(param, "SetNewPurpose");
            result = 1;
            message = "Purpose Updated Successfully";
            return Json(new { result = result, message = message }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult PurposeCorrection()
        {
        ...
        public ActionResult Create(SpecialLoanCollectionViewModel Model)
        {
            try
            {
                ...
                var entity = Mapper.Map<SpecialLoanCollectionViewModel, DailyLoanTrx>(Model);
                var p = productService.GetById(loanser.ProductID);
                string pro;
                pro = p.ProductCode;
                if (ModelState.IsValid)
                {
                    if (pro.Substring(0, 1) != "7")
                    {
