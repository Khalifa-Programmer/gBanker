 ...
 private void MapDropDownList(MemberNomineeSavingViewModel model)
        {
            ...
            //Gender Type
            var genderypeList = new List<SelectListItem>();
            genderypeList.Add(new SelectListItem() { Text = "Male", Value = "M", Selected = true });
            genderypeList.Add(new SelectListItem() { Text = "Female", Value = "F" });
            genderypeList.Add(new SelectListItem() { Text = "Trangender", Value = "T" });
            model.GenderTypeList = genderypeList;
        }
        ...
        [HttpPost]
        public JsonResult SaveMemberNomineeInformation(MemberNomineeSavingViewModel memberNominee, string base64image, string base64imageFingerThumb)
        {
            long result = 0;
            ...
            try
            {
                ...
                {
                    var isDuplicate = memberNomineeService
                        .GetMany(p => p.IsActive == true && p.MemberId == memberNominee.MemberId && p.NomineeName
                        .ToUpper() == memberNominee.NomineeName
                        .ToUpper());
                    ...
                    else
                    {
                        ...
                        entity.Gender = memberNominee.Gender;
                ...
                else
                {
                    ...
                    var isDuplicate =
                            memberNomineeService
                            .GetMany(
                                p => p.IsActive == true && p.NomineeName
                                    .ToUpper() == memberNominee.NomineeName
                                    .ToUpper() && p.MemberId == memberNominee.MemberId && p.MemberNomineeId != memberNominee.MemberNomineeId
                                );
                    ...
                    else
                    {
                        ...
                        entityUpdate.Gender = memberNominee.Gender;
        ...
        //    var memOffcDesigList = ultimateReportService.GetDataWithParameter(param, "dbo.SP_GetMemberNomineeList"); //SP_GetEmployeeNomineeList
        ...
        public ActionResult UploadNomineeImage(HttpPostedFileBase file, string MemberNomineeId)
        {
            ...
            string nomineeImageDirectory = Server.MapPath("~/Uploads/Nominee/Image/");
            string nomineeImagePath = "~/Uploads/Nominee/Image/";

            //If directory not exists then create new directory
            if (!Directory.Exists(nomineeImageDirectory))
            {
                Directory.CreateDirectory(nomineeImageDirectory);
            }

            // NomineeImagePath upload as file
            if (file != null && file.ContentLength > 0)
            {
                ...
                var shortGuid = Convert.ToBase64String(Guid.NewGuid().ToByteArray()).Replace("/", "_").Replace("+", "-").Substring(0, 5);
                var imgName = $"imgfile_{MemberNomineeId}_{shortGuid}";
                string fileName = imgName + Path.GetExtension(file.FileName);
                string imagePath = Path.Combine(nomineeImageDirectory, fileName);
                file.SaveAs(imagePath);

                entity.NomineeImagePath = nomineeImagePath + fileName;
        ...
        public ActionResult UploadNomineeSignatureImage(HttpPostedFileBase file, string MemberNomineeId)
        {
            ...
            //Path Directory in project folder
            string nomineeSignDirectory = Server.MapPath("~/Uploads/Nominee/Signature/");
            string nomineeSignPath = "~/Uploads/Nominee/Signature/";

            //If directory not exists then create new directory
            if (!Directory.Exists(nomineeSignDirectory))
            {
                Directory.CreateDirectory(nomineeSignDirectory);
            }
            ...
            if (file != null && file.ContentLength > 0)
            {
                ...
                var shortGuid = Convert.ToBase64String(Guid.NewGuid().ToByteArray()).Replace("/", "_").Replace("+", "-").Substring(0, 5);
                var imgName = $"signfile_{MemberNomineeId}_{shortGuid}";
                string fileName = imgName + Path.GetExtension(file.FileName);
                string thumbImagePath = Path.Combine(nomineeSignDirectory, fileName);
                file.SaveAs(thumbImagePath);

                entity.NomineeSignaturePath = nomineeSignPath + fileName;
                memberNomineeService.Update(entity);
                Result = 1;
            }
            else
        ...
        public ActionResult RetrieveNomineeImage(int id)
        {
            ...
            byte[] imagePath;
            if (string.IsNullOrEmpty(NomineeDetail.NomineeImagePath))
            {
            ...
            {
                string path = NomineeDetail.NomineeImagePath;
        ...
        public ActionResult RetrieveNomineeSignatureImage(int id)
        {
            ...
            byte[] signPath;
            if (string.IsNullOrEmpty(NomineeDetail.NomineeSignaturePath))
            {
                string strImgPathAbsolute = HttpContext.Server.MapPath("~/images/blank-headshot.jpg");
                Image img = Image.FromFile(strImgPathAbsolute);
            ...
            {
                string path = NomineeDetail.NomineeSignaturePath;
                string dir = Server.MapPath(path);
            ...
            return File(signPath, "image/jpg");
        }
